...
# this script was sourced from: https://github.com/naikai/sake/blob/master/R/format_utils.R ; credit to github user: naikai
...
detect_genecnt_platform <- function(data, method="mean"){
  ...
  if(method == "mean"){
    select.fun <- function(x){
      df2 <- data.frame(sapply(x, function(n) as.numeric(as.character(n))))  # convert to numeric
      rowMeans(df2)
    } 
  }else if(method == "sd"){
